# 9BizClaw v2.4 — Hướng Dẫn Sử Dụng & Tính Năng Sản Phẩm

> Tài liệu sản phẩm dành cho bot hỗ trợ khách hàng 9BizClaw. Xử lý sự cố xem tài liệu riêng: 9bizclaw-support-kb.md
...
| Kỹ năng có sẵn + tùy chỉnh | Vận hành, marketing, theo ngành (BĐS, F&B, spa...) |
...
**Menu chính (icon rail, theo thứ tự từ trên xuống):**
- **Tổng quan** — trang chủ, thống kê, hoạt động
- **Chat** — nhắn trực tiếp với bot trong app
- **Kênh** — mở rộng thành sub-tabs: Telegram, Zalo, Facebook, Google (có chấm trạng thái kết nối)
- **Nội dung** — mở rộng: Tài liệu, Tính cách bot, Skills, Tài sản hình ảnh, AI Models, Lịch tự động
- **Brain** — đồ thị tri thức (knowledge graph) hiện toàn bộ khách hàng, nhóm, tài liệu dưới dạng node
- **Cấu hình** — giao diện Sáng/Tối/Hệ thống, thu nhỏ xuống tray, nâng cao, kiểm tra cập nhật
...
### Tab "Brain" (Đồ thị tri thức)

Knowledge graph hiện toàn bộ entities dưới dạng node trên canvas tương tác:

- **5 loại node:** Khách hàng (vàng), Nhóm (tím), Tài liệu (đỏ), Học hỏi (xám), Kỹ năng (tím nhạt)
- **Toolbar:** Chip lọc theo loại + thanh tìm kiếm + nút refresh
- **Tương tác:** Zoom (scroll/nút +/-), kéo pan, click node mở panel chi tiết bên phải
- **Panel chi tiết:** Tên, loại, liên kết (node liên quan), nội dung preview, nút mở trong tab tương ứng
- **Thống kê:** Hiện tổng nodes + edges ở góc dưới trái
- **Auto-build:** Bot tự xây dựng graph khi có khách hàng, nhóm, tài liệu
...
- **Sao lưu dữ liệu (encrypted)** — Sao lưu toàn bộ workspace thành 1 file mã hóa bằng mật khẩu
- **Khôi phục từ backup** — Phục hồi từ file backup (cần mật khẩu khi sao lưu)
...
Từ v2.4.4, Zalo DM sử dụng mô hình allowlist — chỉ những người trong danh sách mới nhận được phản hồi từ bot. Quản lý danh sách cho phép qua tab Bạn bè trên Dashboard. Toggle bật/tắt từng người.
...
Bot có kỹ năng có sẵn chia 3 nhóm. Hệ thống tự kích hoạt kỹ năng phù hợp khi nhận tin nhắn — CEO không cần bật/tắt thủ công.
...
| 5 | Quản lý kênh | Tạm dừng/tiếp tục Telegram + Zalo, quản lý allowlist |
...
Kỹ năng tùy chỉnh CEO tạo sẽ hiện ở tab Skills bên cạnh kỹ năng có sẵn.
...
> Chi tiết xử lý sự cố: xem tài liệu **9bizclaw-support-kb.md** (mục A-M, 60+ triệu chứng có keyword tra cứu).
