...
**Est:** 2.5-3 tuần dev + test + ship (10-14 business days)
**Author:** devops@modoro.com.vn
**Date:** 2026-04-16
**Revision:** 4 (R3.1 + preload shim for code-level blocklist/dedup/system-msg)
...
- **Preload shim** (`electron/patches/whatsapp-shim.js`) via Node `--require` at gateway spawn:
  - Code-level blocklist gate (drop inbound + drop outbound to blocked JIDs)
  - Code-level sender dedup (per-sender Map, TTL 3s)
  - Code-level system-msg filter (drop baileys `messageStubType` group events before agent dispatch)
...
- **Shim module signature drift**: preload shim detects plugin exports by function name (`sendMessageWhatsApp`, inbound handler). If openclaw upgrade renames exports, shim silently stops gating. Mitigation: smoke test verifies exports exist at expected names; CI fails build if missing. Existing installs → shim logs warning to `logs/whatsapp-shim.log` + fires `sendCeoAlert` on first boot where detection fails.
- **Pause queue on resume**: if plugin queues inbound when `dmPolicy=disabled`, resume fires burst. Mitigated by: pause mechanism uses file-based check inside shim wrapper (NOT dmPolicy flip) — accept that baileys WS keeps running during pause, but shim drops all inbound until `whatsapp-paused.json` expires. Verified via O-C test Day 1.
...
Electron main (MODOROClaw)
  └── Gateway subprocess: node --require <shim> openclaw.mjs gateway run
        └── whatsapp-shim.js (Module._load hook — monkey-patches plugin)
              ├── telegram plugin (untouched)
              ├── openzalo plugin (untouched — has own TS-source patches)
              └── @openclaw/whatsapp (native config + shim-wrapped send/inbound)
...
electron/patches/
└── whatsapp-shim.js                 # preload shim — shipped in app bundle
...
└── config/
    ...
    ├── whatsapp-blocklist.json      # SHIM reads this — code-level gate
    ├── whatsapp-group-settings.json # __default + per-group override
    ├── whatsapp-paused.json         # SHIM reads this — code-level gate
...
```
WhatsApp server → baileys WS → plugin inbound handler
  → SHIM-WRAPPED inbound:
      ...
      - system-msg check (baileys messageStubType ∈ group events) → drop
      - dedup check (per-sender Map, TTL 3s) → drop if duplicate
  → native allowFrom + groupRequireMention checks (plugin)
  → dispatch to agent runtime
  → agent reply
  → plugin sanitizeAssistantVisibleText → SHIM-WRAPPED sendMessageWhatsApp:
...
### 5.7b. Preload shim — code-level gate for blocklist/dedup/system-msg

File: `electron/patches/whatsapp-shim.js` — shipped bundled (electron-builder `files` already includes `scripts/**/*`; add `patches/**/*`).

**Gateway spawn:**
```js
// main.js _startOpenClawImpl — existing gateway spawn
const shimPath = path.join(app.getAppPath(), 'patches', 'whatsapp-shim.js');
const args = ['--require', shimPath, openClawCliJs, 'gateway', 'run'];
// ... existing spawn(absoluteNode, args, { shell: false, env: enrichedEnv })
```

Shim runs BEFORE openclaw loads. Uses `Module._load` hook to wrap exports as modules load.

**Detection strategy (content-hashed filenames):**

Plugin imports are content-hashed (e.g., `../../send-BJuTARpY.js`). Filename changes every openclaw release. Shim detects by:

1. **Function name match**: after `_load` returns, inspect exports for function names `sendMessageWhatsApp`, `monitorWebInbox`, etc.
2. **Fallback by signature**: if name renamed, match by parameter count + nearby symbols (e.g., first-arg regex `/^to|jid|target$/`).

```js
// electron/patches/whatsapp-shim.js (sketch)
const Module = require('module');
const origLoad = Module._load;
const workspace = process.env.MODORO_WORKSPACE || require('os').homedir();

// In-memory state
const _dedupMap = new Map();  // key: senderJid+body, value: timestamp
let _shimApplied = false;

function readJson(relPath, fallback) {
  try {
    const fs = require('fs');
    const p = require('path').join(workspace, relPath);
...
function isBlocked(jid) {
  ...
  const list = readJson('config/whatsapp-blocklist.json', []);
  return list.includes(norm) || list.includes(String(jid));
}

function isPaused() {
  const p = readJson('config/whatsapp-paused.json', null);
...
function isReadMode() {
  try {
    const fs = require('fs');
    const mode = fs.readFileSync(require('path').join(workspace, 'config/whatsapp-mode.txt'), 'utf8').trim();
    return mode === 'read';
  } catch { return false; }
}

// baileys messageStubType enum values (stable for years)
const GROUP_STUB_TYPES = new Set([20, 21, 22, 24, 25, 27, 28, 29, 30, 31, 32]);

function isSystemEvent(msg) {
  const stubType = msg?.messageStubType;
  return typeof stubType === 'number' && GROUP_STUB_TYPES.has(stubType);
}

function isDuplicate(senderJid, body) {
  ...
  // Prune: if > 500 entries, drop oldest
  if (_dedupMap.size > 500) {
    const cutoff = now - 60000;
    for (const [k, v] of _dedupMap) if (v < cutoff) _dedupMap.delete(k);
  }
  return false;
}
...
Module._load = function(request, parent, isMain) {
  ...
  // Wrap outbound sendMessageWhatsApp
  if (m && typeof m.sendMessageWhatsApp === 'function' && !m.__modoroWrapped) {
    const orig = m.sendMessageWhatsApp;
    m.sendMessageWhatsApp = async function(to, body, opts) {
      if (isBlocked(to)) {
        console.log('[modoro-shim] drop outbound to blocked jid:', to);
        return { dropped: 'blocklist' };
      }
      return orig.call(this, to, body, opts);
    };
    m.__modoroWrapped = true;
    _shimApplied = true;
  }

  // Wrap inbound handler (function name TBD from impl phase — likely monitorWebInbox or onMessage)
  // [implementation details resolved in impl plan after reading login-*.js]

  return m;
};

// Boot signal — log to workspace
try {
  const fs = require('fs');
  const path = require('path');
  fs.appendFileSync(
    path.join(workspace, 'logs', 'whatsapp-shim.log'),
    `${new Date().toISOString()} shim preloaded, waiting for plugin import\n`
  );
} catch {}

// 60s after gateway boot, if _shimApplied still false → alert
setTimeout(() => {
  if (!_shimApplied) {
    try {
      const fs = require('fs');
      ...
      fs.appendFileSync(
        path.join(workspace, 'logs', 'whatsapp-shim.log'),
        `${new Date().toISOString()} WARNING: shim never hooked — openclaw exports changed?\n`
      );
    } catch {}
  }
}, 60000).unref();
...
**Safety properties:**
- Idempotent via `__modoroWrapped` flag on module
- No source file mutation → plugin intact for `@openclaw/whatsapp` auto-update
- Failure mode = soft: if detection fails, shim logs warning, gateway boots normally with native-only gate; CEO sees "blocklist not active" warning in Dashboard
- Shim runs with same `process.env.MODORO_WORKSPACE` as gateway → workspace paths resolve correctly
...
**Week 1 — Foundation + shim (5 ngày):**
- Day 1: Close O-A (QR), O-B (probe shape), O-C (pause via shim), O-D (shim detection) — manual tests on vendor plugin
- Day 2: `electron/patches/whatsapp-shim.js` implement + unit tests (blocklist, dedup, system-msg, pause, mode=read)
- Day 3: `ensureDefaultConfig` schema + gateway spawn with `--require` + in-process config helpers
...
- Day 8: AGENTS.md v45 backup rules + memory handlers + view modal
- Day 9: Watchdog integration + preload bridges + mode/pause UI
- Day 10: Integration tests (shim detection smoke, end-to-end blocklist, pause queue)

**Week 3 — E2E + ship (3-5 ngày):**
- Day 11-12: 72h soak on dev account + E2E on volunteer CEO account
- Day 13: Fix findings, final smoke test
- Day 14: Ship v2.4.0 (EXE + Mac DMG)

**Total: 2.5-3 tuần** (was 2 tuần in R3).
