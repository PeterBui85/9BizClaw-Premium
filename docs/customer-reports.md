...
- **Fix (`electron/lib/google-api.js`, gogEnv):** chọn backend theo nền tảng — **Windows → `auto`** (gog tự chọn Credential Manager; Windows không khoá ACL theo binary nên gog unsigned đọc được; không có dấu `:`), **macOS/Linux → `file`** (giữ nguyên fix Keychain ở trên). Guard `check-google-workspace-audit-fixes.js` cập nhật theo nền tảng + allowlist giá trị hợp lệ.
- **CẢNH BÁO — gog chỉ nhận `auto|keychain|file`:** lần fix đầu mình đặt `wincred` (tên backend của 99designs/keyring) → gog báo `invalid keyring backend: "wincred"` → vẫn hỏng Windows. Guard cũ chỉ kiểm tra CHUỖI gogEnv trả về (không chạy gog thật) nên không bắt được. Kiểm chứng matrix trên gog v0.13.0 thật: `auto`→chạy (trả về danh sách lịch), `keychain`→"not available", `file`→lỗi colon, `wincred`→invalid. Bài học: test phải SPAWN gog thật với env của gogEnv, không chỉ assert chuỗi.
