...
- POST /api/google/sheets/create body: {title, parent?} — tạo Sheet trống
- POST /api/google/sheets/create-formatted body: {title, headers, data?, style?, textColumns?, parent?} — tạo Sheet có format sẵn. style: "standard"|"minimal". textColumns: mảng index cột wrap text (VD: [4] cho cột E). Headers tự bold + nền xanh + freeze.
...
**QUY TẮC: Khi tạo Sheet mới, LUÔN dùng `/sheets/create-formatted` thay vì `/sheets/create` + ghi data thủ công.** Truyền headers + data + textColumns cho cột dài (VD caption, mô tả). Sheet ra sẽ có header bold nền xanh, freeze hàng đầu, cột text wrap tự động.
