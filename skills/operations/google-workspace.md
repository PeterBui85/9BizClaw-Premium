...
Pattern tạo mới:
1. Tạo `.xlsx` local bằng Node package `xlsx` bundled, **qua skill-runner** (`POST /api/skill/test-exec` {runtime:"node"}) — ghi binary thẳng bằng `XLSX.writeFile(wb, "<absolute path>")`. Set header, độ rộng cột, wrap text/format cơ bản ngay trong file. **KHÔNG ghi `.xlsx` qua `write_file`/`/api/file/write` dạng text — sẽ hỏng file.**
