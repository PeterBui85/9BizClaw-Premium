...
- POST /api/google/sheets/create body: {title, sheets?, parent?} — tạo Google Sheet mới
- POST /api/google/sheets/format body: {spreadsheetId, range, formatJson, formatFields} — format cells (CellFormat JSON + field mask)
- POST /api/google/sheets/freeze body: {spreadsheetId, rows?, cols?, sheet?} — freeze dòng/cột
- POST /api/google/sheets/number-format body: {spreadsheetId, range, type} — định dạng số (NUMBER|CURRENCY|PERCENT|DATE|TIME|TEXT)
...
- "cột SĐT dạng text" → POST /api/google/sheets/number-format type=TEXT
