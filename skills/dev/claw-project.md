...
**Internal APIs:** Cron API (port 20200) — localhost-only HTTP server for cron CRUD, FB schedule, Google Workspace, file ops. Bearer token auth (48 hex, rotated every boot).
...
| `cron-api.js` | HTTP API server (port 20200), auth gate, Google routes, FB routes, exec, workspace R/W |
| `fb-schedule.js` | FB two-phase scheduling (generate + publish), CEO approval flow, cross-midnight handling |
| `fb-publisher.js` | Facebook Graph API calls, rate limiter, queue timeout |
...
| File | Responsibility |
|------|---------------|
| `dashboard.html` | Main dashboard — overview, channels, knowledge, cron, FB, settings |
...
For PUBLIC endpoints (no auth): add to `PUBLIC_ROUTES` Set. For Zalo-safe read endpoints: add to Google-style pre-gate pattern. Default: everything requires Bearer token.
...
| 6 | Facebook Auto-Post | fb-schedule.js, fb-publisher.js | Schedule → generate → preview → approve → publish. Cross-midnight |
| 7 | Knowledge + RAG | dashboard-ipc.js, workspace.js, knowledge.js | Upload → extract → summarize → index → search. DB singleton |
| 8 | License + Wizard | license.js, wizard.html, license.html | Key verify → seal → wizard steps → boot. Machine fingerprint |
