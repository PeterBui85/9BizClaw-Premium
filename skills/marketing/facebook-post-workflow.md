...
1. Gọi `GET /api/fb/pages` — nhận danh sách `[{id, pageId, pageName, shortName, enabled, tokenStatus}]`
...
```
web_fetch url="http://127.0.0.1:20200/api/fb/verify?pageId=<pageId>" method=GET
```
...
| "đăng đi" / "ok" / "gửi đi" | Gọi `/api/fb/post?preview=true&pageId=<pageId>` → lấy nonce → gọi `/api/fb/post?approvalNonce=...&pageId=<pageId>` → báo kết quả |
