...
CEO nêu tên page (VD "đăng lên cf1", "đăng lên cafe") → **truyền thẳng `page=<tên hoặc tên ngắn đó>`** vào API (`/api/fb/verify`, `/api/fb/post`, `/api/fb/insights`). Server tự khớp tên ngắn (chính xác, case-insensitive) rồi tên Facebook (substring). KHÔNG cần gọi `/api/fb/pages` rồi tự match `pageId` — đó là việc của server.

Xử lý khi server trả lỗi:
- `Không tìm thấy fanpage "X"` → gọi `GET /api/fb/pages`, báo CEO: "Không tìm thấy fanpage '[X]'. Các fanpage hiện có: [danh sách]"
- `Nhiều fanpage khớp "X": ...` → HỎI CEO: "Có nhiều fanpage khớp: [danh sách]. Anh muốn đăng lên page nào?"
- `Fanpage "X" đang tắt` → "Fanpage '[X]' đã tắt. Bật lại trong Dashboard > Facebook."

CEO KHÔNG nêu tên + chỉ có 1 page enabled → gọi `GET /api/fb/pages`, dùng page đó, xác nhận: "Đăng lên **[pageName]**?". Lịch đăng (`/api/fb/schedule/create`) cần `targetPageId` → lấy `pageId` từ `GET /api/fb/pages`.
...
```
web_fetch url="http://127.0.0.1:20200/api/fb/verify?page=<tên fanpage>" method=GET
```
...
| "đăng đi" / "ok" / "gửi đi" | Gọi `/api/fb/post?preview=true&page=<tên fanpage>` → lấy nonce → gọi `/api/fb/post?approvalNonce=...&page=<tên fanpage>` → báo kết quả |
...
```
web_fetch "http://127.0.0.1:20200/api/fb/schedule/create?postTime=09:00&leadMinutes=60&targetPageId=<pageId>&prompt=<prompt-tạo-ảnh>&caption=<caption>&label=<tên>&imageSize=1024x1024"
```

Lịch một lần (1 ngày cụ thể) — thêm `postDate`:
```
web_fetch "http://127.0.0.1:20200/api/fb/schedule/create?postTime=09:00&postDate=2026-06-15&targetPageId=<pageId>&prompt=<prompt>&caption=<caption>&label=Bài ngày 15/6"
```
