...
foreach ($t in $batch) {
    ...
    try {
        ...
        $results += @{ n=$t.n; model=$t.model; pt=$promptTk; reply=$reply; short=$replyShort; err=$null }
    } catch {
        $results += @{ n=$t.n; model=$t.model; pt=0; reply=""; short=""; err=$_.Exception.Message }
    }
}
...
$jsonResults | ConvertTo-Json -Depth 3 | Out-File -FilePath "C:\Users\buitu\Desktop\claw\.test-results-$BatchStart-$BatchEnd.json" -Encoding utf8
"Done batch $BatchStart-$BatchEnd"
