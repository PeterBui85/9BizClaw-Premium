...
export async function handleOpenzaloInbound(params: {
  ...
  if (rawBody && !__cbIsInternal) {
    ...
    const __cbPatterns: RegExp[] = [
      ...
      /\bread\s+(?:file|folder|directory|log|config)\b/i,
      /\bwrite\s+(?:file|to\s+file)\b/i,
      /\bdelete\s+(?:file|folder)\b/i,
      /\bopen\s+(?:file|folder|desktop)\b/i,
      /\bdownload\s+(?:file|from)\b/i,
      /(?:đọc|xem|mở)\s+(?:file|tệp|tập tin|thư mục|folder)/i,
      /(?:doc|xem|mo)\s+(?:file|tep|tap tin|thu muc|folder)/i,
      /(?:ghi|tạo|viết)\s+(?:file|tệp|tập tin)/i,
      /(?:ghi|tao|viet)\s+(?:file|tep|tap tin)/i,
      /(?:xóa|xoá)\s+(?:file|tệp|tập tin|thư mục)/i,
      /(?:xoa)\s+(?:file|tep|tap tin|thu muc)/i,
      /(?:chạy|thực thi)\s+(?:lệnh|command|script)/i,
      /(?:chay|thuc thi)\s+(?:lenh|command|script)/i,
    ];
