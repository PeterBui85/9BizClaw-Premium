...
const googleApiSource = fs.readFileSync(path.join(__dirname, '..', 'lib', 'google-api.js'), 'utf8');
assert('gmail attachment uses security gateway', googleApiSource.includes("require('./attachment-security')"), 'google-api must use attachment-security');
assert('gmail attachment returns agent-safe attachment', googleApiSource.includes('toAgentAttachment'), 'downloadGmailAttachment should return sanitized attachment metadata');
