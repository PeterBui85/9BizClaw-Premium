...
const facebookWorkflow = fs.readFileSync(path.join(root, 'skills', 'marketing', 'facebook-post-workflow.md'), 'utf8');
...
assert('facebook workflow polls slow image jobs', facebookWorkflow.includes('waitMs=300000') && facebookWorkflow.includes('timedOut: true') && facebookWorkflow.includes('/api/image/status?jobId=<jobId>'), 'Facebook workflow must not treat slow image generation as final failure');
