...
function sbFetch(path, method, body, extraHeaders) {
  return new Promise((resolve) => {
    ...
    req.setTimeout(SUPABASE_TIMEOUT_MS, () => { try { req.destroy(); } catch {} resolve({ status: 0, body: 'timeout', parsed: null }); });
    // Hard 15s timeout — covers body-stall scenario where headers arrive but body hangs
    setTimeout(() => { try { req.destroy(); } catch {} }, 15000);
...
// Shared APPDATA path computation — deduplicates identical logic in
// getMachineId(), licensePath(), and migrateWorkspaceLicense().
...
/**
 * Get or compute the machine fingerprint (SHA-256 of hostname + MAC + platform).
 * Cached for process lifetime. Persisted to %APPDATA%/9bizclaw/.machine-id.
 * @returns {string} 32-char hex machine fingerprint
 */
function getMachineId() {
  if (_cachedMachineId) return _cachedMachineId;
  const fs = require('fs');
  const path = require('path');
  const midFile = path.join(_getAppDataDir(), '9bizclaw', '.machine-id');
  try {
...
function licensePath() {
  const path = require('path');
  return path.join(_getAppDataDir(), '9bizclaw', 'license.json');
}
...
function migrateWorkspaceLicense() {
  const fs = require('fs');
  const path = require('path');
  const newPath = path.join(_getAppDataDir(), '9bizclaw', 'license.json');
...
/**
 * Verify an Ed25519-signed license key offline.
