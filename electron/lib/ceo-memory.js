...
function _normalizeScope(scope, channel) {
  ...
  if (ch === 'zalo' || ch === 'whatsapp') return 'customer';
  return 'ceo';
}
...
function _rowAllowedForActor(row, channel, actorId) {
  ...
  const isCustomerEntity = ['customer', 'zalo_user', 'whatsapp_user', 'zalo_group', 'whatsapp_group', 'group'].includes(entityType);
...
async function searchMemory(query, { limit = 5, bumpRelevance = true, scopes = null, channel = null, actorId = null, includePending = false, includeDisabled = false } = {}) {
  ...
  if (_isCustomerChannel(channel)) {
    ...
    where.push(`(
      NOT (
        LOWER(COALESCE(entity_type, '')) IN ('customer', 'zalo_user', 'whatsapp_user', 'zalo_group', 'whatsapp_group', 'group')
